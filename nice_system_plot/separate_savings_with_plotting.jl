...
# **Step 4: Extract Names & Numbers for Only Connected Nodes**
connected_vertices = [(Zone_Name[i], Zone_Number[i]) for i in 1:length(Zone_Name) if Zone_Number[i] in connection_zones]
# Convert to DataFrame
vertices_df = DataFrame(Line_Name=first.(connected_vertices), Zone_Number=last.(connected_vertices))
# Save only connected vertices
CSV.write(joinpath(@__DIR__,"filtered_vertices.csv"), vertices_df)

# filter out inactive 
filtered_df = df[Active .> 0, :]

# Drop the first two columns (Zone_Name and Zone_Number)
filtered_edges_df = select(filtered_df, Not(column_indices[1:2]))


# Save filtered edges
CSV.write(joinpath(@__DIR__,"filtered_edges.csv"), filtered_edges_df)
