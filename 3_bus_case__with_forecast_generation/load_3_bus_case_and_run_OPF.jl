...
#Functions
function update_battery_state_of_charge(
    current_stage_of_charge, # current state of charge in pu
    charge_P, # power of charging of battery
    charge_Q, # charge reactive power
    discharge_P, # power of discharging of battery
    discharge_Q, # discharge reactive power
    time_interval) # time interval as fraction of hour
    charge_S = sqrt(charge_P^2 + charge_Q^2) 
    discharge_S = sqrt(discharge_P^2 + discharge_Q^2) 
    next_element = current_stage_of_charge[end] + (charge_S - discharge_S) * time_interval
    push!(current_stage_of_charge,next_element)
    return current_stage_of_charge
end
...
function generate_storage_components(sys::System, storage_operations::Array{Tuple{Int, Float64, Float64}})
    ...
    for (i,storage) in enumerate(bat)
        ...
        storage_id = i
        efficiency_in = storage.efficiency.in # input, charge efficiency
        efficiency_out = storage.efficiency.out # output, discharge efficiency
        ...
        charge_power = storage_entry[1][2]  # pu of charging power
        discharge_power = storage_entry[1][3]  # pu of discharging power
        ...
        if charge_power > 0
            push!(additional_loads, PowerLoad(;
                ...
                active_power = charge_power/efficiency_in,  # apply PU
                reactive_power = 0.0,
        ...
        if discharge_power > 0
            push!(additional_generators, ThermalStandard(;
                ...
                active_power_limits = (min = 0.0, max = discharge_power/efficiency_out), # 6 MW to 30 MW per-unitized by device base_power
                reactive_power_limits =(min = storage.reactive_power_limits.min, max = storage.reactive_power_limits.max), #nothing, # Per-unitized by device base_power
...
main_direct_with_data = "C:/Users/andre/Desktop/Julia_code_main_directory/Small_case_redo_forecast"
DF_load1 = CSV.read(joinpath(main_direct_with_data,"optimization_results","DF_load1.csv"), DataFrame)
DF_load2 = CSV.read(joinpath(main_direct_with_data,"optimization_results","DF_load2.csv"), DataFrame)
DF_wind1 = CSV.read(joinpath(main_direct_with_data,"optimization_results","DF_wind1.csv"), DataFrame)
DF_battery_out = CSV.read(joinpath(main_direct_with_data,"optimization_results","DF_battery_out.csv"), DataFrame)
DF_battery_in = CSV.read(joinpath(main_direct_with_data,"optimization_results","DF_battery_in.csv"), DataFrame)
DF_gas_gen = CSV.read(joinpath(main_direct_with_data,"optimization_results","DF_gas_gen.csv"), DataFrame)
...
battery_in_array = abs.(DF_battery_in[:, 1])  # Extract first column as an array
gas_gen_array = abs.(DF_gas_gen[:,1])
# Convert and Save
storage_operations = [
    (1, 0.0, 0.1),  # Battery 1 charging 0 MW, discharging 100 MW
]

system_adress = joinpath(main_direct_with_data,"3_bus_test/test_system.json")
...
adress_m_system = joinpath(main_direct_with_data,"3_bus_test/matpower_3_bus.m")
...
bat_P = []
bat_Q = []
state_of_charge = [0.11 * 500] # Initial state of charge
...
for interest = 1:N
    ...
    network_data["load"]["2"]["pd"] = load2_array[interest]+battery_in_array[interest]

    network_data["gen"]["3"]["pmax"] = wind1_array[interest]
    #network_data["gen"]["2"]["pmax"] = gas_gen_array[interest]
    ...
    #add elements of solution
    gas_genP = vcat(gas_genP, generator_data[2,2]) # add P of gas generator
    gas_genQ = vcat(gas_genQ, generator_data[2,3]) # add Q of gas generator
    wind_genP = vcat(wind_genP, generator_data[3,2]) # add P of wind generator
    wind_genQ = vcat(wind_genQ, generator_data[3,3]) # add P of wind generator
    bat_P = vcat(bat_P, generator_data[1,2]) # add P of battery
    bat_Q = vcat(bat_Q, generator_data[1,3]) # add Q of battery
    timing = 1 #fraction of hour
    update_battery_state_of_charge(state_of_charge, battery_in_array[interest], 0, bat_P[end], bat_Q[end], timing)
end

println(state_of_charge)
df_summary = DataFrame(Pbat = bat_P, Qbat = bat_Q, Pgas = gas_genP, Qgas = gas_genQ, Pwind = wind_genP, Qwind = wind_genQ, State_of_charge = state_of_charge[2:end])
CSV.write(joinpath(main_direct_with_data,"optimization_results","AC_solution.csv"),df_summary)