...
DF_battery_in = CSV.read(joinpath(main_direct_with_data,"DF_battery_in.csv"), DataFrame)
...
println(gas_genP)
df_summary = DataFrame(Pbat = bat_P, Qbat = bat_Q, Pgas = gas_genP, Qgas = gas_genQ, Pwind = wind_genP, Qwind = wind_genQ)
CSV.write(joinpath(main_direct_with_data,"AC_solution.csv"),df_summary)
